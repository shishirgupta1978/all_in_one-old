...
class Task_register(models.Model):
    ...
    journal = models.ForeignKey(Journal,on_delete=models.CASCADE, related_name="task_journal")
    taskname= models.ForeignKey(Taskname,on_delete=models.CASCADE, related_name="task_taskname")
    ...
    def __str__(self):
        return str(self.id)+"-"+":"+" ::"+self.status+"("+str(self.user)+")"
    def filename(self):
